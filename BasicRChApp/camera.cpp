...
OrbitCamera::OrbitCamera(XMFLOAT3 target, float minDist, float maxDist, float dist)
	: m_angleX(0), m_angleY(0), m_target(target.x, target.y, target.z, 1.0f), m_distance(0)
{
...
XMMATRIX OrbitCamera::getViewRotMatrix() const {
  return XMMatrixRotationY(-m_angleY) * XMMatrixRotationX(-m_angleX) *
         XMMatrixTranslation(0, 0, m_distance);
}
