...
DxApplication::DxApplication(HINSTANCE hInstance, int wndWidth, int wndHeight,
    ...
    : WindowApplication(hInstance, wndWidth, wndHeight, wndTitle),
      ...
      m_camera(XMFLOAT3(0, 0.0f, 0.0f), 0.01f, 50.0f, 5),
      //m_camera(XMFLOAT3(0.0f, 4.0f, -3.0f)),
      m_viewport{m_window.getClientSize()} {
  ID3D11Texture2D* temp = nullptr;
  auto hr = m_device.swapChain()->GetBuffer(0, __uuidof(ID3D11Texture2D),
                                            reinterpret_cast<void**>(&temp));
  const dx_ptr<ID3D11Texture2D> backTexture{temp};
  if (FAILED(hr)) THROW_DX(hr);
...
void DxApplication::Render() {
  ...
  m_device.context()->ClearDepthStencilView(
      m_depthBuffer.get(), D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}

void mini::DxApplication::UpdateBuffer(const dx_ptr<ID3D11Buffer>& buffer,
                                       const void* data, size_t count) {
  D3D11_MAPPED_SUBRESOURCE res;
  auto hr = m_device.context()->Map(buffer.get(), 0, D3D11_MAP_WRITE_DISCARD, 0,
                                    &res);
  if (FAILED(hr)) THROW_DX(hr);
