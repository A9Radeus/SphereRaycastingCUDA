...
namespace mini
{
	...
	using AdjEdges = std::tuple<unsigned short, unsigned short, unsigned short, unsigned short>;
	...
	public:
		...
		template<typename VertexType>
    static Mesh AdjacencyTriMesh(const DxDevice& device,
    ...
    static std::vector<VertexPosition> RectangleVertsPos(float width, float height);
		...
		//Mesh Loading
    static Mesh LoadMesh(const DxDevice& device,
                          const std::wstring& meshPath);

    static Mesh LoadDuck(const DxDevice& device, const std::wstring& meshPath);
